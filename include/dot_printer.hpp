...
public:
    ...
    void Visit(const AST::FunctionNode& functionNode) const override {
        os_ << SET_NODE << &functionNode
            << SET_MRECORD_SHAPE
            << SET_LABEL  << "FUNC"    << SET_ADR  << &functionNode << END_LABEL
            << SET_FILLED << SET_COLOR << std::hex << AST::dump_style::SCOPE_NODE_COLOR << std::dec
            << END_NODE;

        os_ << SET_NODE << &functionNode << SET_LINK << SET_NODE << functionNode.get_body() << std::endl;
        functionNode.get_body()->accept(*this);
    }

    void Visit(const AST::CallNode& callNode) const override {
        os_ << SET_NODE << &callNode
            << SET_MRECORD_SHAPE
            << SET_LABEL  << "CALL"    << SET_ADR  << &callNode << END_LABEL
            << SET_FILLED << SET_COLOR << std::hex << AST::dump_style::BINARYOP_NODE_COLOR << std::dec
            << END_NODE;

        os_ << SET_NODE << &callNode << SET_LINK << SET_NODE << callNode.get_callee() << std::endl;
        callNode.get_callee()->accept(*this);

        for (auto* argumentExpr : callNode.get_args()) {
            os_ << SET_NODE << &callNode << SET_LINK << SET_NODE << argumentExpr << std::endl;
            argumentExpr->accept(*this);
        }
    }

    void Visit(const AST::ReturnNode& returnNode) const override {
        os_ << SET_NODE << &returnNode
            << SET_MRECORD_SHAPE
            << SET_LABEL  << "RETURN"  << SET_ADR  << &returnNode << END_LABEL
            << SET_FILLED << SET_COLOR << std::hex << AST::dump_style::PRINT_NODE_COLOR << std::dec
            << END_NODE;

        if (auto* returnExpr = returnNode.get_expr()) {
            os_ << SET_NODE << &returnNode << SET_LINK << SET_NODE << returnExpr << std::endl;
